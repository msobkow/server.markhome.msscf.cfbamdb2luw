// Description: Java 11 DB/2 LUW Jdbc DbIO implementation for HostNode.
...
/*
 *	CFBamDb2LUWHostNodeTable DB/2 LUW Jdbc DbIO implementation
 *	for HostNode.
 */
	...
	implements ICFBamHostNodeTable
{
		...
		Long priorHostNodeId )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			final String sql = "CALL sp_page_hostnode_all( ?, ?, ?, ?, ?, ?, ? )";
		...
		Long priorHostNodeId )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			final String sql = "CALL sp_page_hostnode_by_clusteridx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + ", "
				+		"?" + " )";
		...
		CFSecHostNodeBuff Buff )
	{
		...
		if( "a002".equals( Buff.getClassCode() )
