// Description: Java 11 DB/2 LUW Jdbc DbIO implementation for SecSession.
...
/*
 *	CFBamDb2LUWSecSessionTable DB/2 LUW Jdbc DbIO implementation
 *	for SecSession.
 */
	...
	implements ICFBamSecSessionTable
{
	...
	throws SQLException
	{
		...
		int idxcol = 1;
		CFSecSecSessionBuff buff = schema.getFactorySecSession().newBuff();		buff.setRequiredSecSessionId( CFBamDb2LUWSchema.convertUuidString( resultSet.getString( idxcol ) ) );
		idxcol++;
		...
		UUID priorSecSessionId )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			final String sql = "CALL sp_page_secsess_all( ?, ?, ?, ?, ?, ? )";
		...
		UUID priorSecSessionId )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			final String sql = "CALL sp_page_secsess_by_secuseridx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " )";
		...
		UUID priorSecSessionId )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			final String sql = "CALL sp_page_secsess_by_secdevidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + ", "
				+		"?" + " )";
		...
		UUID priorSecSessionId )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			final String sql = "CALL sp_page_secsess_by_finishidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"TIMESTAMP_FORMAT( ?, 'YYYY-MM-DD-HH24.MI.SS' )" + ", "
				+		"?" + " )";
			...
			if( Finish != null ) {
				stmtPageBuffByFinishIdx.setString( argIdx++, CFBamDb2LUWSchema.getTimestampString( Finish ) );
			}
			else {
		...
		UUID priorSecSessionId )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			final String sql = "CALL sp_page_secsess_by_secproxyidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " )";
		...
		CFSecSecSessionBuff Buff )
	{
		...
		if( "a010".equals( Buff.getClassCode() )
