// Description: Java 11 DB/2 LUW Jdbc DbIO implementation for ISOLang.
...
/*
 *	CFBamDb2LUWISOLangTable DB/2 LUW Jdbc DbIO implementation
 *	for ISOLang.
 */
public class CFBamDb2LUWISOLangTable
	implements ICFBamISOLangTable
{
	...
	public CFBamDb2LUWISOLangTable( CFBamDb2LUWSchema argSchema ) {
		schema = argSchema;
	}
	...
	public CFSecISOLangBuff readDerivedByIdIdx( CFSecAuthorization Authorization,
		short ISOLangId )
	{
		final String S_ProcName = "CFBamDb2LUWISOLangTable.readDerivedByIdIdx() ";
		CFSecISOLangBuff buff;
		if( ! schema.isTransactionOpen() ) {
			throw new CFLibUsageException( getClass(),
				S_ProcName,
	...
	public CFSecISOLangBuff readDerivedByCode3Idx( CFSecAuthorization Authorization,
		String ISO6392Code )
	{
		final String S_ProcName = "CFBamDb2LUWISOLangTable.readDerivedByCode3Idx() ";
		CFSecISOLangBuff buff;
		if( ! schema.isTransactionOpen() ) {
			throw new CFLibUsageException( getClass(),
				S_ProcName,
		...
		CFSecISOLangBuff Buff )
	{
		...
		if( "a007".equals( Buff.getClassCode() )
			&& ( ! schema.isSystemUser( Authorization ) ) )
		{
			throw new CFLibRuntimeException( getClass(),
				S_ProcName,
				"Permission denied -- only system user can modify ISOLang data" );
		}
		ResultSet resultSet = null;
		try {
		...
		String argISO6392Code )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			final String sql = "CALL sp_delete_iso_lang_by_code3idx( ?, ?, ?, ?, ?" + ", "
				+		"?" + " )";
