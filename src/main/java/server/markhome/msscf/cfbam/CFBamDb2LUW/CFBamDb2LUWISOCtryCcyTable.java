// Description: Java 11 DB/2 LUW Jdbc DbIO implementation for ISOCtryCcy.
...
/*
 *	CFBamDb2LUWISOCtryCcyTable DB/2 LUW Jdbc DbIO implementation
 *	for ISOCtryCcy.
 */
public class CFBamDb2LUWISOCtryCcyTable
	implements ICFBamISOCtryCcyTable
{
	...
	public CFBamDb2LUWISOCtryCcyTable( CFBamDb2LUWSchema argSchema ) {
		schema = argSchema;
	}
	...
	public CFSecISOCtryCcyBuff readDerivedByIdIdx( CFSecAuthorization Authorization,
		...
		short ISOCcyId )
	{
		final String S_ProcName = "CFBamDb2LUWISOCtryCcyTable.readDerivedByIdIdx() ";
		CFSecISOCtryCcyBuff buff;
		if( ! schema.isTransactionOpen() ) {
			throw new CFLibUsageException( getClass(),
				S_ProcName,
		...
		short ISOCtryId )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			final String sql = "CALL sp_read_iso_cntryccy_by_ctryidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + " )";
		...
		short ISOCcyId )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			final String sql = "CALL sp_read_iso_cntryccy_by_ccyidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + " )";
		...
		CFSecISOCtryCcyBuff Buff )
	{
		...
		if( "a005".equals( Buff.getClassCode() )
			&& ( ! schema.isSystemUser( Authorization ) ) )
		{
			throw new CFLibRuntimeException( getClass(),
				S_ProcName,
				"Permission denied -- only system user can modify ISOCtryCcy data" );
		}
		ResultSet resultSet = null;
		try {
			short ISOCtryId = Buff.getRequiredISOCtryId();
		...
		short argISOCtryId )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			final String sql = "CALL sp_delete_iso_cntryccy_by_ctryidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + " )";
		...
		short argISOCcyId )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			final String sql = "CALL sp_delete_iso_cntryccy_by_ccyidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + " )";
