// Description: Java 11 DB/2 LUW Jdbc DbIO implementation for SubProject.
...
/*
 *	CFBamDb2LUWSubProjectTable DB/2 LUW Jdbc DbIO implementation
 *	for SubProject.
 */
	...
	implements ICFBamSubProjectTable
{
		...
		long TopProjectId )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			final String sql = "CALL sp_read_sprjdef_by_topprojectidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " )";
		...
		CFIntSubProjectBuff Buff )
	{
		...
		if( "a105".equals( Buff.getClassCode() )
		...
		long argTopProjectId )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			final String sql = "CALL sp_delete_sprjdef_by_topprojectidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " )";
