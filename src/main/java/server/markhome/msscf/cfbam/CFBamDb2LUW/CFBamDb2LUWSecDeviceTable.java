// Description: Java 11 DB/2 LUW Jdbc DbIO implementation for SecDevice.
...
/*
 *	CFBamDb2LUWSecDeviceTable DB/2 LUW Jdbc DbIO implementation
 *	for SecDevice.
 */
	...
	implements ICFBamSecDeviceTable
{
	...
	public CFSecSecDeviceBuff readDerivedByNameIdx( CFSecAuthorization Authorization,
		...
		String DevName )
	{
		final String S_ProcName = "CFBamDb2LUWSecDeviceTable.readDerivedByNameIdx() ";
		CFSecSecDeviceBuff buff;
		if( ! schema.isTransactionOpen() ) {
			throw new CFLibUsageException( getClass(),
				S_ProcName,
		...
		String priorDevName )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			final String sql = "CALL sp_page_secdev_all( ?, ?, ?, ?, ?, ?, ? )";
		...
		String DevName )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			final String sql = "CALL sp_read_secdev_by_nameidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " )";
		...
		String priorDevName )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			final String sql = "CALL sp_page_secdev_by_useridx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + ", "
				+		"?" + " )";
		...
		String argDevName )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			final String sql = "CALL sp_delete_secdev_by_nameidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " )";
