// Description: Java 11 DB/2 LUW Jdbc DbIO implementation for SecGroup.
...
/*
 *	CFBamDb2LUWSecGroupTable DB/2 LUW Jdbc DbIO implementation
 *	for SecGroup.
 */
	...
	implements ICFBamSecGroupTable
{
		...
		boolean IsVisible )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			final String sql = "CALL sp_read_secgrp_by_clustervisidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " )";
		...
		CFSecSecGroupBuff Buff )
	{
		...
		if( "a00c".equals( Buff.getClassCode() )
		...
		boolean argIsVisible )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			final String sql = "CALL sp_delete_secgrp_by_clustervisidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " )";
			...
			if( argIsVisible ) {
				stmtDeleteByClusterVisIdx.setString( argIdx++, "Y" );
			}
			else {
				stmtDeleteByClusterVisIdx.setString( argIdx++, "N" );
			}
